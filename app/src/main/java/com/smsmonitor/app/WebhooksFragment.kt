...
class WebhooksFragment : Fragment() {
    ...
    private fun showEditDialog(webhook: WebhookConfig?) {
        ...
        // === Test Webhook button ===
        val btnTest = view.findViewById<MaterialButton>(R.id.btnTestWebhook)
        val cardTestResult = view.findViewById<com.google.android.material.card.MaterialCardView>(R.id.cardTestResult)
        val tvTestResult = view.findViewById<TextView>(R.id.tvTestResult)

        btnTest.setOnClickListener {
            val testUrl = etUrl.text?.toString()?.trim() ?: ""
            if (testUrl.isBlank()) {
                etUrl.error = "Enter a URL first"
                return@setOnClickListener
            }

            val testHttpMethod = when {
                chipGet.isChecked -> "GET"
                chipPut.isChecked -> "PUT"
                else -> "POST"
            }
            val testContentType = if (chipForm.isChecked) "application/x-www-form-urlencoded" else "application/json"
            val testAuthType = when {
                chipAuthNone.isChecked -> "none"
                chipAuthBearer.isChecked -> "bearer"
                chipAuthCustom.isChecked -> "custom_header"
                else -> "api_key"
            }
            val testApiKey = etApiKey.text?.toString()?.trim() ?: ""
            val testAuthHeaderName = etAuthHeaderName.text?.toString()?.trim() ?: "X-API-Key"
            val testSecret = etSecret.text?.toString()?.trim() ?: ""
            val testBodyTemplate = etBodyTemplate.text?.toString()?.trim() ?: ""
            val headersText = etCustomHeaders.text?.toString()?.trim() ?: ""
            val testCustomHeaders = mutableMapOf<String, String>()
            if (headersText.isNotEmpty()) {
                for (line in headersText.lines()) {
                    val colonIdx = line.indexOf(':')
                    if (colonIdx > 0) {
                        testCustomHeaders[line.substring(0, colonIdx).trim()] = line.substring(colonIdx + 1).trim()
                    }
                }
            }
            ...
            Thread {
                try {
                    val result = WebhookSender.testWebhook(testConfig)
                    btnTest.post {
                        ...
                        // Color the card border based on result
                        val isSuccess = result.startsWith("\u2713")
                        cardTestResult.strokeColor = if (isSuccess) 0xFF4CAF50.toInt() else 0xFFEF5350.toInt()
                    }
                } catch (e: Exception) {
                    btnTest.post {
                        btnTest.isEnabled = true
                        btnTest.text = "\uD83D\uDD14  Test Webhook"
                        tvTestResult.text = "\u2717 Unexpected Error\n${e.javaClass.simpleName}: ${e.message}"
                        cardTestResult.visibility = View.VISIBLE
                        cardTestResult.strokeColor = 0xFFEF5350.toInt()
                    }
                }
            }.start()
        }
