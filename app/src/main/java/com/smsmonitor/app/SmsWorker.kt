            ...
            cursor.use { c ->
                while (c.moveToNext()) {
                    ...
                    if (payment != null) {
                        ...
                        val status = WebhookSender.sendToMatchingWebhooks(payment, -1, appContext)
                        when (status) {
                            "sent" -> { db.markPaymentSent(payment, id); count++ }
                            "idle" -> db.markPaymentIdle(payment)
                        }
                    }
                }
            }
    ...
    private fun retryPendingPayments() {
        try {
            ...
            for (record in pending) {
                ...
                val success = WebhookSender.sendToMatchingWebhooks(payment, -1, appContext)
                if (success == "sent") {
                    db.markPaymentSent(payment)
                    Log.w(TAG, "Retry success: ${record.trxid}")
                } else if (success == "idle") {
                    db.markPaymentIdle(payment)
                }
            }
