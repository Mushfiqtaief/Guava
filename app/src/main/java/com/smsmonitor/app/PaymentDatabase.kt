...
data class PaymentRecord(
    ...
    val detectedAt: Long,
    val sentAt: Long,
    val simSlot: Int = -1       // 0=SIM1, 1=SIM2, -1=unknown
)
...
class PaymentDatabase(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    companion object {
        ...
        private const val DB_VERSION = 3  // Added sim_slot column
        ...
        private const val COL_SIM_SLOT = "sim_slot"
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("""
            CREATE TABLE $TABLE (
                ...
                $COL_SIM_SLOT INTEGER DEFAULT -1
            )
        """.trimIndent())
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 3) {
            // Add sim_slot column (preserves existing data)
            try {
                db.execSQL("ALTER TABLE $TABLE ADD COLUMN $COL_SIM_SLOT INTEGER DEFAULT -1")
                Log.w(TAG, "Database upgraded: added sim_slot column")
            } catch (e: Exception) {
                // Column might already exist, or table needs recreating
                Log.w(TAG, "Migration failed, recreating: ${e.message}")
    ...
    /**
     * Mark a payment as "idle" (no matching webhook for this message).
     */
    fun markPaymentIdle(payment: SmsParser.Payment): Boolean {
        return try {
            val db = writableDatabase
            val values = ContentValues().apply {
                put(COL_STATUS, "idle")
            ...
            val updated = db.update(TABLE, values, "$COL_TRXID = ?", arrayOf(payment.trxid))
            if (updated > 0) {
                Log.w(TAG, "Payment marked idle: ${payment.trxid}")
            }
            updated > 0
        } catch (e: Exception) {
            Log.e(TAG, "Error marking idle: ${e.message}")
            false
        }
    }
        ...
        try {
            ...
            val cursor = db.query(
                ...
                arrayOf(COL_ID, COL_TRXID, COL_METHOD, COL_AMOUNT, COL_STATUS,
                    COL_SMS_BODY, COL_SMS_ADDRESS, COL_DETECTED_AT, COL_SENT_AT, COL_SIM_SLOT),
                null, null, null, null,
                "$COL_ID DESC"
            )
            cursor.use { c ->
    ...
    fun getIdleCount(): Int = getCountByStatus("idle")
