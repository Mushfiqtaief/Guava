...
class WebhookStore(context: Context) {
    ...
    fun getAll(): List<WebhookConfig> {
        ...
        return try {
            ...
            // Don't nuke existing config on parse error — return empty list instead
            android.util.Log.e("WebhookStore", "Failed to parse webhooks JSON: ${e.message}")
            emptyList()
        }
    }
    ...
    private fun seedDefaults(): List<WebhookConfig> {
        // Seed with an empty sample webhook on first run
