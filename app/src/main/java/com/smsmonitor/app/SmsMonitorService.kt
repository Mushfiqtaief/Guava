...
class SmsMonitorService : Service() {
    ...
    private fun handlePayment(payment: SmsParser.Payment, smsId: Long = 0L, smsBody: String = "", smsAddress: String = "", simSlot: Int = -1) {
        // Insert into DB as pending first
        paymentDb.insertPayment(payment, smsBody, smsAddress, smsId)
        DebugLog.w(TAG, "Inserted ${payment.trxid} as PENDING, sending to webhooks (SIM${simSlot+1})...")

        serviceScope.launch {
            val success = WebhookSender.sendToMatchingWebhooks(payment, simSlot, this@SmsMonitorService)
            val status = if (success) "sent" else "pending"

            DebugLog.w(TAG, ">>> ${payment.method.uppercase()} ${payment.amount}TK [${payment.trxid}] [$status]")

            // Update DB status
            if (success) {
                paymentDb.markPaymentSent(payment, smsId)
            }
    ...
    private fun subIdToSlot(subId: Int): Int {
        ...
        return try {
            ...
            // Some devices use subId as slot index directly
